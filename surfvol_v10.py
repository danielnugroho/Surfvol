...
-m Datasets//PYRAMID//PYRAMID.TIF -b Datasets//PYRAMID//BASE.TIF -e DatasetsvPYRAMID//EXTENTS.SHP -n -32767
...
-m Datasets//SPILES2//OCTOBER.TIF -b Datasets//SPILES2//NOVEMBER.TIF -e Datasets//SPILES2//TOES.SHP -n -32767

VERSION HISTORY:

v.10-231203 - improvements
    features:
        - ability to prepare tabular report
...
with rio.open(modelpath) as ras1, rio.open(basepath) as ras2:
    ...
    if (xres1*yres1) > (xres2*yres2):
        ...
        ras1, ras2 = ras2, ras1  # Swap ras1 and ras2 using tuple unpacking
        swapped = True
    else:
...
print("Summary of volume within the intersection of surfaces:")
print("Area         : " + str(round(area_all,3)))
...
# check if there are more than 1 polygon
if len(gdf) > 1:
    
    # if yes, prepare a detailed report
    report = []
    
    # construct boundary array of 0 and 1 from the check result
    # the 1s means it's within boundary and should be calculated
    # do it for each polygon in the shapefile
    for index, polygon in enumerate(gdf['geometry']):
        
        arraybnd = features.rasterize([polygon], out_shape=array1.shape,
        ...
        report.append([gdf['NAME'][index], area_bnd, volume_bnd, cut_bnd, fill_bnd])
    
    # make a pandas dataframe of the report for easier printing
    df = pd.DataFrame(report, columns=['BOUNDARY', 'AREA', 'NET VOL', 'CUT VOL', 'FILL VOL'])
    print("  Detailed Volume Calculation Report")
    print(df.round(3))
    print()

# construct boundary array of 0 and 1 from the check result
# the 1s means it's within boundary and should be calculated
# do it for all polygons in the shapefile, as a summary
...
print("Summary of volume within specified boundary:")
print("Area         : " + str(round(area_bnd,3)))
